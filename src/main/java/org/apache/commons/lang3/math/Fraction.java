...
public final class Fraction extends Number implements Comparable<Fraction> {
    ...
    private static int greatestCommonDivisor(int u, int v) {
        ...
        // For every fork if, while we assign an ID
        // Decision 1
        // B1: condition true enter block 
        // B2: contition false skip
        if (u == 0 || v == 0) {
            branchCoverage[1] = true; // B1 hit
            // Decision 2 
            ...
            if (u == Integer.MIN_VALUE || v == Integer.MIN_VALUE) {
                ...
                branchCoverage[4] = true; // B4 add missing else
            }
            return Math.abs(u) + Math.abs(v);
        } else {
            branchCoverage[2] = true; // B2 add missing else
        }
        //Decision 3
